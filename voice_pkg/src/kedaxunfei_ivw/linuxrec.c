...
// int set_mic_volume_to_100_percent() {
//     snd_mixer_t *handle;
//     snd_mixer_selem_id_t *sid;
//     const char *card = "sysdefault";
//     const char *selem_name = "Capture"; // 注意: 这个名称可能需要根据您的具体硬件进行调整
//     long max_volume;

//     // 打开混音器
//     if (snd_mixer_open(&handle, 0) < 0) return -1;
//     if (snd_mixer_attach(handle, card) < 0) return -1;
//     if (snd_mixer_selem_register(handle, NULL, NULL) < 0) return -1;
//     if (snd_mixer_load(handle) < 0) return -1;
...
//     // 获取并设置最大音量
//     snd_mixer_selem_get_capture_volume_range(elem, NULL, &max_volume);
//     snd_mixer_selem_set_capture_volume_all(elem, max_volume);
				...
				void* user_cb_para)
{
  ...
  // // 调整麦克风音量至100%
  // if (set_mic_volume_to_100_percent() != 0) {
  //     free(myrec);
  //     return -1; // 处理错误
  // }
