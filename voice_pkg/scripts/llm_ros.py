...
class MainStream():
    def __init__(self) -> None:
        ...
        # self.speechindex = ['火箭展厅', '卫星展厅', '航天器展厅', '运载火箭展厅']
        self.speechindex = ['火箭展厅', '卫星展厅']
    ...
    def run(self):
        ...
        allspeech = {
            '火箭展厅':'''在您右手边的是中国研制的“长征系列”运载火箭家族模型。 “长征一号” 是我国第一种运载火箭型号， 1970年4月24日成功地将 “东方红一号”卫星送入预定轨道，奠定了“长征”系列火箭发展的基础。''',
            '卫星展厅':'''这是东方红一号卫星模型， 其中一根天线为实物， 因当时共做了 5颗样星，第一颗卫星就发射成功，所以可以保存一根实物天线。''',
            '航天器展厅':'''人造地球卫星是环绕地球,''',
            '运载火箭展厅':'''这是我国发射的第一颗返回式卫星,'''
        }

        text2speech('大家好，我来带领大家参观航天馆！', index=1000)
        thread.start_new_thread(self.navigate, (None,))

        for locid, key in enumerate(self.speechindex):  #TODO  def get_next_destination():
            while locid < self.current_location:
                locid += 1

            # 调用导航服务，监听导航状态，如果到达一个目标点，self.arrival置True
            self.arrival = False  #TODO  [0,0,0,1]

            text2speech("下面带大家参观"+key, index=0)
            self.navigate(locid+1)
            ...
            print(f"\n机器人到达目标点: {self.current_location}")
    ...
    def interrupt(self):
        ...
        if 'qa' in action:
            # 只做问答
            action = self.multi_qa(text_rewrited)
            if action == 'visit':
                return
            if self.ifinterrupt:
                textconfirm = text2speech("好的，那我继续讲解了", index=1000)
            else:
                if self.current_location  < len(self.speechindex):
                    textconfirm = text2speech("好的，那我带领大家参观下一个展厅", index=1000)
        elif 'sleep' in action:
            pass
        elif 'visit' in action:
            target_point = action.split(" ")[1]
            if '下一个' in target_point:
                if self.current_location < len(self.speechindex):
                    self.current_location += 1
                else:
                    text2speech("我们已经在最后一个展厅了", index=1000)
            elif '上一个' in target_point:
                if self.current_location > 0:
                    self.current_location -= 1
                else:
                    text2speech("我们已经在第一个展厅了", index=1000)
            target_index = self.speechindex.index(target_point)
            print("\ntarget_index: ", target_index)
            self.current_location = target_index
        else:
            # 继续
            if self.ifinterrupt:  # 讲解的时候被打断
                textconfirm = text2speech("好的，那我继续讲解了", index=1000)
            else:
                if self.current_location  < len(self.speechindex):
                    textconfirm = text2speech("好的，那我带领大家参观下一个展厅", index=1000)
    ...
    def intent_recognition(self, data):
        ...
        # 打断并录音之后返回的文本，给到ppl
        time_before_ppl = datetime.now()
        ppl_bool = get_ppl_bool(text)
        time_after_ppl = datetime.now()
        print("\nPPL耗时:", time_after_ppl - time_before_ppl)
        text2tts = None
        if ppl_bool == False:
            # ppl太大了
        ...
        if not continue_flag:
            ...
            print("useword: ", userword)
            if not userword:
                return None, None

        # 根据用户的话进行意图识别
        print("根据用户的话进行意图识别: ", text2tts)
        text2speech("好的，我听到了。", index=0)
    ...
    def multi_qa(self, userword):
        ...
        for i in range(self.qaepoch):
            ...
            userword = self.pardon(pardon_round=2) # 反复录制直至用户语音不为空，最多重复pardon_round轮
            msg = String(data=userword)
            action, userword = self.intent_recognition(msg)    # TODO:interrupt
            print("action, userword:", action, userword)
            if action == 'qa':
    ...
    def pardon(self, pardon_round):  # TODO:utils.py
