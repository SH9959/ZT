...
class CWaitForMessage:
    ...
    def wait_for_messages(self):
        ...
        # gyro_sub = message_filters.Subscriber("/camera/gyro/sample", msg_Imu)
        # accel_sub = message_filters.Subscriber("/camera/accel/sample", msg_Imu)
        # pos_sub = message_filters.Subscriber("/camera/odom/sample", msg_Odometry)
        # topic synchronization by time
        # ts = message_filters.ApproximateTimeSynchronizer([img_sub, depth_sub, gyro_sub, accel_sub, pos_sub], queue_size=10, slop=0.5, allow_headerless=True)
        ts = message_filters.ApproximateTimeSynchronizer([img_sub], queue_size=10, slop=0.5, allow_headerless=True)
